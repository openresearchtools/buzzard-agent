...
cache=${BUZZARD_AGENT_CACHE_ROOT:-"$component_dir/.cache"}
...
if lock["platform"] != "linux-x64":
    raise SystemExit("unsupported runtime-lock platform")
node = lock["node"]
for key, value in {
...
case "$(uname -m)" in
  x86_64|amd64) ;;
  *) echo 'the pinned runtime supports linux-x64 only' >&2; exit 1 ;;
esac
