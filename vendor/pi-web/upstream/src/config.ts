...
export function defaultPiWebConfigPath(env: NodeJS.ProcessEnv = process.env): string {
  ...
  return join(xdgConfigHome !== undefined && xdgConfigHome !== "" ? xdgConfigHome : join(homedir(), ".config"), "wildbuzzard", "agent", "config.json");
}

export function defaultPiWebDataDir(env: NodeJS.ProcessEnv = process.env): string {
  const xdgDataHome = env["XDG_DATA_HOME"];
  return join(xdgDataHome !== undefined && xdgDataHome !== "" ? xdgDataHome : join(homedir(), ".local", "share"), "wildbuzzard", "agent");
}
...
export function piWebDataDir(env: NodeJS.ProcessEnv = process.env, cwd = process.cwd()): string {
  ...
  if (configured === undefined || configured === "") return defaultPiWebDataDir(env);
...
export function examplePiWebConfig(config: PiWebConfig = {}): string {
  return `${JSON.stringify({ host: config.host ?? "127.0.0.1", port: config.port ?? 8765, allowedHosts: config.allowedHosts ?? [] }, null, 2)}\n`;
}
