...
import { localMachine, MachineService, type CreateMachineInput, type UpdateMachineInput } from "./machineService.js";
import { wildbuzzardServiceIdentity } from "../wildbuzzardServiceIdentity.js";

export function registerLocalOnlyMachineRoutes(app: FastifyInstance, machines = new MachineService()): void {
  app.get("/api/machines", () => ({ machines: [localMachine()] }));
  app.get<{ Params: { machineId: string } }>("/api/machines/:machineId/health", async (request, reply) => {
    if (request.params.machineId !== "local") return reply.code(404).send({ error: "Machine not found" });
    const health = await machines.health("local");
    const challenge = request.headers["x-wildbuzzard-agent-challenge"];
    const identity = wildbuzzardServiceIdentity(typeof challenge === "string" ? challenge : undefined, request.raw.socket);
    return identity === undefined ? health : { ...health, serviceIdentity: identity };
  });
  app.get<{ Params: { machineId: string }; Querystring: { refresh?: string } }>("/api/machines/:machineId/runtime", async (request, reply) => {
    if (request.params.machineId !== "local") return reply.code(404).send({ error: "Machine not found" });
    return await machines.runtime("local", request.query.refresh === "1");
  });
  app.get<{ Params: { machineId: string } }>("/api/machines/:machineId", async (request, reply) => {
    if (request.params.machineId !== "local") return reply.code(404).send({ error: "Machine not found" });
    return localMachine();
  });
  app.post("/api/machines", (_request, reply) => reply.code(403).send({ error: "WildBuzzard Agent only connects to its bundled local runtime" }));
  app.patch("/api/machines/:machineId", (_request, reply) => reply.code(403).send({ error: "WildBuzzard Agent only connects to its bundled local runtime" }));
  app.delete("/api/machines/:machineId", (_request, reply) => reply.code(403).send({ error: "WildBuzzard Agent only connects to its bundled local runtime" }));
}
