...
const serviceRefs: Record<ServiceId, NativeServiceRef> = {
  sessiond: {
    id: "sessiond",
    ...nativeServiceManagerRefs.sessiond,
  },
  web: {
    id: "web",
    ...nativeServiceManagerRefs.web,
  },
  uiDev: {
    id: "uiDev",
    ...nativeServiceManagerRefs.uiDev,
  },
};
...
async function nativeServiceCommands(): Promise<NativeServiceCommands> {
  ...
  return {
    ...(restartable.length === 0 ? {} : { restart: restartNativeServicesCommand(backend, restartable, "wildbuzzard-agent-restart") }),
    ...(web.length === 0 ? {} : { restartWeb: restartNativeServicesCommand(backend, web, "wildbuzzard-agent-restart-web") }),
    ...(sessiond.length === 0 ? {} : { restartSessiond: restartNativeServicesCommand(backend, sessiond, "wildbuzzard-agent-restart-sessiond") }),
