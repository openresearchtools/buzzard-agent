...
describe("PI WEB status", () => {
  ...
  it.skipIf(process.platform !== "linux")("suggests native systemd commands for local development services", async () => {
    ...
    try {
      ...
      await installSystemdServiceFiles(home, ["wildbuzzard-agent-sessiond.service", "wildbuzzard-agent-ui-dev.service"]);
      const daemon = daemonWithRuntime(runningSessiondRuntime());

      const status = await getPiWebStatus(daemon);

      expect(status.commands.restart).toBe("systemd-run --user --collect --unit=wildbuzzard-agent-restart -- systemctl --user restart wildbuzzard-agent-ui-dev.service wildbuzzard-agent-sessiond.service");
      expect(status.commands.restartWeb).toBe("systemd-run --user --collect --unit=wildbuzzard-agent-restart-web -- systemctl --user restart wildbuzzard-agent-ui-dev.service");
      expect(status.commands.restartSessiond).toBe("systemd-run --user --collect --unit=wildbuzzard-agent-restart-sessiond -- systemctl --user restart wildbuzzard-agent-sessiond.service");
      expect(status.messages.find((message) => message.id === "sessiond-stale")?.command).toBe("systemd-run --user --collect --unit=wildbuzzard-agent-restart-sessiond -- systemctl --user restart wildbuzzard-agent-sessiond.service");
    } finally {
      await Promise.all([
