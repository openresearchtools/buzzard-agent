...
import { registerLocalOnlyMachineRoutes, registerMachineRoutes } from "./machines/machineRoutes.js";
...
export async function buildApp(deps: AppDependencies = {}): Promise<FastifyInstance> {
  ...
  const localOnly = deps.localOnly ?? process.env["WILDBUZZARD_AGENT_LOCAL_ONLY"] === "1";
  ...
  app.get<{ Params: { pluginId: string; "*": string } }>("/pi-web-plugins/:pluginId/*", async (request, reply) => {
    if (!localOnly && await proxyMachinePluginAsset(machines, request.params.pluginId, request.params["*"], request.url, reply)) return;
  ...
  if (localOnly) {
    registerLocalOnlyMachineRoutes(app, machines);
  } else {
  ...
  if (!localOnly) registerMachineProxyRoutes(app, machines);
