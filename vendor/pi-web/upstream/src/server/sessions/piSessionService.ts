 ...
 * and PI WEB's sections land after them. Pi's built-in extension factories
 * are included so SDK sessions expose the same built-ins as the Pi CLI.
 */
export async function piWebResourceLoaderOptions(
  appendSystemPromptSections: readonly string[],
): Promise<NonNullable<CreateAgentSessionServicesOptions["resourceLoaderOptions"]>> {
...
function createDefaultRuntimeFactory(
  ...
  const resourceLoaderOptionsPromise = piWebResourceLoaderOptions(appendSystemPromptSections);
  return async ({ cwd, agentDir, sessionManager, sessionStartEvent, initialModel, initialThinkingLevel, delegationToolsEnabled }) => {
    const resourceLoaderOptions = await resourceLoaderOptionsPromise;
...
export class PiSessionService implements SessionRouteService {
  ...
  applyAuthChange(change: AuthChange = {}): void {
    // AuthService emits only for committed mutations. Pi normally synchronizes
    // the runtime first; its post-commit failure path is retried by the catalog
    // refresher listener, without putting a refresh back on this request path.
