...
describe("defaultSessionArchiveFilePath", () => {
  ...
  it("uses the WildBuzzard data directory when PI_WEB_DATA_DIR is unset", () => {
    expect(defaultSessionArchiveFilePath({}, "/tmp/pi-web")).toBe(join(homedir(), ".local", "share", "wildbuzzard", "agent", "archived-sessions.json"));
  });
});
