...
interface InitialStateWaiter {
  resolve: (state: OAuthFlowState) => void;
  timer?: TimerHandle;
}
...
interface OAuthFlowRecord {
  ...
  initialStateWaiters: Set<InitialStateWaiter>;
...
export interface OAuthLoginFlowServiceOptions {
  ...
  runningTtlMs?: number;
  /** Maximum time start APIs wait for the first UI-visible state. */
  initialStateWaitMs?: number;
  now?: () => number;
...
const DEFAULT_INITIAL_STATE_WAIT_MS = 250;
...
export class OAuthLoginFlowService {
  ...
  private readonly initialStateWaitMs: number;
  ...
  constructor(options: OAuthLoginFlowServiceOptions = {}) {
    ...
    this.initialStateWaitMs = options.initialStateWaitMs ?? DEFAULT_INITIAL_STATE_WAIT_MS;
  ...
  start(options: {
    ...
    void options.runtime.login(options.providerId, options.authType ?? "oauth", interaction).then(
      ...
      (error: unknown) => {
        ...
        if (isCommittedLoginError(error, options.providerId)) {
          ...
          void this.reconcileCommittedLogin(record, options.onComplete);
          return;
        }
  ...
  /** Resolves on the first UI-visible state, or the current running snapshot at the configured bound. */
  waitForFirstObservableState(flowId: string): Promise<OAuthFlowState> {
    const record = this.flows.get(flowId);
    if (record === undefined) return Promise.reject(new Error("Login flow not found"));
    if (isObservableState(record.state) || this.initialStateWaitMs <= 0) return Promise.resolve(cloneState(record.state));

    return new Promise((resolve) => {
      const waiter: InitialStateWaiter = { resolve };
      record.initialStateWaiters.add(waiter);
      waiter.timer = setTimeout(() => { this.resolveInitialStateWaiter(record, waiter); }, this.initialStateWaitMs);
      unrefTimer(waiter.timer);
      if (isObservableState(record.state)) this.resolveInitialStateWaiter(record, waiter);
    });
  }
  ...
  dispose(): void {
    for (const record of this.flows.values()) {
      ...
      const pending = this.clearPending(record);
      pending?.reject(new Error("Login cancelled"));
      this.resolveInitialStateWaiters(record);
    }
    this.flows.clear();
  }
  ...
  // ModelRuntime persists before synchronization. Success or its matching
  // post-commit error must supersede a transient cancelled state.
  private async reconcileCommittedLogin(record: OAuthFlowRecord, onComplete?: () => void | Promise<void>): Promise<void> {
    if (record.completionStarted) return;
    record.completionStarted = true;
  ...
  private updateState(record: OAuthFlowRecord, state: OAuthFlowState): void {
    record.state = state;
    if (isObservableState(state)) this.resolveInitialStateWaiters(record);
  }

  private resolveInitialStateWaiters(record: OAuthFlowRecord): void {
    for (const waiter of [...record.initialStateWaiters]) this.resolveInitialStateWaiter(record, waiter);
  }

  private resolveInitialStateWaiter(record: OAuthFlowRecord, waiter: InitialStateWaiter): void {
    if (!record.initialStateWaiters.delete(waiter)) return;
    if (waiter.timer !== undefined) clearTimeout(waiter.timer);
    waiter.resolve(cloneState(record.state));
  }
...
function isObservableState(state: OAuthFlowState): boolean {
  return state.status !== "running" || state.prompt !== undefined || state.select !== undefined || state.auth !== undefined || state.progress.length > 0 || (state.info?.length ?? 0) > 0;
}

function isCommittedLoginError(error: unknown, providerId: string): error is CredentialSynchronizationError {
  return error instanceof CredentialSynchronizationError && error.operation === "login" && error.providerId === providerId;
}
