...
describe("OAuthLoginFlowService", () => {
  ...
  it("waits across a queued runtime boundary for the first observable state", async () => {
    vi.useFakeTimers();
    const beginLogin = deferred<undefined>();
    const service = new OAuthLoginFlowService({ initialStateWaitMs: 100 });
    const state = service.start({
      providerId: "test-provider",
      providerName: "Test Provider",
      runtime: fakeRuntime(async (_providerId, interaction) => {
        await beginLogin.promise;
        await interaction.prompt({ type: "secret", message: "Enter API key" });
      }),
      authType: "api_key",
    });
    const firstState = service.waitForFirstObservableState(state.flowId);

    expect(vi.getTimerCount()).toBe(2);
    beginLogin.resolve(undefined);
    await expect(firstState).resolves.toMatchObject({
      status: "running",
      prompt: { message: "Enter API key", promptType: "secret" },
    });
  ...
  it("returns the valid running snapshot when the initial-state wait reaches its bound", async () => {
    vi.useFakeTimers();
    const never = deferred<undefined>();
    const service = new OAuthLoginFlowService({ initialStateWaitMs: 100, runningTtlMs: 1000 });
    const state = service.start({
      providerId: "test-provider",
      providerName: "Test Provider",
      runtime: fakeRuntime(() => never.promise),
    });
    const firstState = service.waitForFirstObservableState(state.flowId);

    await vi.advanceTimersByTimeAsync(100);

    await expect(firstState).resolves.toEqual(state);
    expect(vi.getTimerCount()).toBe(1);
    service.dispose();
    expect(vi.getTimerCount()).toBe(0);
  });

  it("settles and clears an initial-state wait when disposed", async () => {
    vi.useFakeTimers();
    const never = deferred<undefined>();
    const service = new OAuthLoginFlowService({ initialStateWaitMs: 100 });
    const state = service.start({
  ...
  it.each([
    {
      name: "a regular login failure",
      error: new Error("Provider login failed"),
      message: "Provider login failed",
    },
    {
      name: "a synchronization error for another provider",
      error: new CredentialSynchronizationError(
        "other-provider",
        "login",
        { type: "api_key", key: "secret" },
        { cause: new Error("sync failed") },
      ),
      message: "Credential login committed for other-provider, but local synchronization failed",
    },
    {
      name: "a synchronization error for another operation",
      error: new CredentialSynchronizationError(
        "test-provider",
        "logout",
        undefined,
      ...
      message: "Credential logout committed for test-provider, but local synchronization failed",
    },
  ])("treats $name as an actual failure", async ({ error, message }) => {
    const onComplete = vi.fn();
    const service = new OAuthLoginFlowService();
    const state = service.start({
      providerId: "test-provider",
      providerName: "Test Provider",
      runtime: { login: () => Promise.reject(error) },
      onComplete,
    });

    await expect(service.waitForFirstObservableState(state.flowId)).resolves.toMatchObject({
      status: "error",
      error: message,
    });
