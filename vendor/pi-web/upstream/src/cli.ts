...
function parseInstallOptions(args: string[]): InstallOptions {
  const options: InstallOptions = { host: "127.0.0.1", port: "8765", mode: "production" };
...
export function configEnvironment(
  ...
  const environment: Record<string, string> = options.config === undefined ? {} : { PI_WEB_CONFIG: configPath };
  for (const key of WILDBUZZARD_SERVICE_ENV_KEYS) {
    const value = env[key];
...
Usage:
  pi-web install [--dev] [--host 127.0.0.1] [--port 8765] [--config ~/.config/wildbuzzard/agent/config.json]
  pi-web uninstall
