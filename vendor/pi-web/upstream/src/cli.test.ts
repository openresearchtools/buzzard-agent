...
describe("configEnvironment", () => {
  it("persists the managed search connection for native services", () => {
    expect(
      configEnvironment(
        { host: "127.0.0.1", port: "5173", mode: "production" },
        "/config.json",
        {
          WILDBUZZARD_SEARCH_CONNECTION_FILE: "/run/user/1000/wildbuzzard/search.json",
          UNRELATED_SECRET: "must-not-leak",
        },
      ),
    ).toEqual({ WILDBUZZARD_SEARCH_CONNECTION_FILE: "/run/user/1000/wildbuzzard/search.json" });
  });
});
...
describe("server plugin recovery restart planning", () => {
  ...
  it("uses the installed native sessiond service without contacting it", () => {
    ...
    expect(plan).toMatchObject({
      kind: "automatic",
      command: "systemctl --user restart wildbuzzard-agent-sessiond.service",
    });
    plan.perform?.();
    expect(runCommand).toHaveBeenCalledWith("systemctl", ["--user", "restart", "wildbuzzard-agent-sessiond.service"]);
  });
  ...
  it("uses the installed launchd session daemon target", () => {
    ...
    expect(plan).toMatchObject({
      kind: "automatic",
      command: "launchctl kickstart -k gui/501/org.wildbuzzard.agent.sessiond",
    });
    plan.perform?.();
    expect(runCommand).toHaveBeenCalledWith("launchctl", ["kickstart", "-k", "gui/501/org.wildbuzzard.agent.sessiond"]);
  });
